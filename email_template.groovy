        ...
        def count = 0
         for(def temp in env3)
         {
                ...
                for (def line in build.getLog(count))
                {
                        ...
                        if(line.contains("http://padlcicdggk4.sw.fortna.net:8088/artifactory/webapp/"))
                        {
...
<%
 // def dir = new File("/var/lib/jenkins/workspace/"+project.name+"/target/surefire-reports")
   def dir = new File(env.JobWorkSpace+'/target/surefire-reports')
   ...
   <!-- Robot Framework Results -->
    ...
    <%  DateFormat format = new SimpleDateFormat("yyyyMMdd HH:mm:ss.SS")
       ...
       suite.caseResults.each() { tc ->
        ...
        <tr><td><br><a href="$build.url">Click Here</a> to view build result</h2><br><h3>Please find below, the build logs and other files.</td></tr></table>
        </span>
